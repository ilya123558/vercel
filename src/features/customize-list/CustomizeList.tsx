...
import { animationImg, animationWithDynamicDalay } from "@/shared/const/animation";

const list = [
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: false, isActive: true},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: false, isActive: true},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: false, isActive: false},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: true, isActive: false},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: true, isActive: false},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: true, isActive: false},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: true, isActive: false},
  {title: 'Energy limit', photo: '/images/modal/zip.png', isDisable: true, isActive: false},
    ...
    <ListWrapper className="mt-[4vw]">
      {openModal && (
        <BuyModal
          ...
          title="Название фона"
          description="Задний фон для главного экрана"
        />
      )}
      <ul className="grid grid-cols-2 gap-[3px]">
        {list.map((item, index) => (
          <li key={index} onClick={handleClick} className="active:scale-[0.98] transition-all will-change-transform p-[1px]">
            <BlockWrapper className={`w-full aspect-square flex items-center flex-col justify-between p-[5.87vw_8vw_3.2vw] !bg-gradient-list-item border ${item.isActive ? 'border-violet' : 'border-[#383A46]'}`}>
              <div className="w-full aspect-square relative rounded-[16px] bg-[#2C2C3B] p-[1vw]">
                <motion.div {...animationImg}>
                  <Image src={item.photo} alt="list-item-img" width={113} height={113} className="w-full h-full" />
                </motion.div>
                {item.isDisable && (
              ...
              <h5 className="font-bold fs-11">{item.title}</h5>
            </BlockWrapper>
          </li>
        ))}
      </ul>
