  ...
  return (
    <Modal open={isOpen} setOpen={setIsOpen}>
      <div className="flex flex-col gap-[2.65vw] min-w-300px">
        <div className="w-300px rounded-[20px] flex items-center flex-col pt-[4vw] bg-dark-violet relative border-[1px] border-dark-blue">
          <button onClick={() => setIsOpen(false)} className='absolute top-15px right-15px'>
          ...
          {subTitle && <p className='fs-10 max-w-190px text-center opacity-[0.5] mt-[7px]'>{subTitle}</p>}
          {imageComponent}
        </div>
        {!disableButton && <Button onClick={onClick} countValue={countValueInButton}>{textButton}</Button>}
        {withCloseBtn && <CloseButton onClick={() => setIsOpen(false)} />}
      </div>
    </Modal>
  );
};